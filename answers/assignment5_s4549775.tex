...
\begin{enumerate}[a]
	...
	\begin{description}
		\item[Integrity] is achieved by appending the MAC to the "enchilada" for which the private key of A is used. An attacker cannot generate a valid MAC for a tampered message.
		\item[Confidentiality] The original datagram (together with some metadata) is encrypted with the public key of B. An attacker cannot decrypt the original payload.
	\end{description}
	\item %c
